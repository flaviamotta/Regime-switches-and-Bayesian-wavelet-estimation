...
wpost_laplace <- function(w, x, s = 1, a = 0.5){
  wpostt <- 1 - (1 - w)/(1 + w * beta.laplace(x, s, a))
  wpostt[round(wpostt, digits = 1)==0] <- 0
  wpostt[1] <- 1
  return(wpostt)
}
...
# ====================== Creating the mixture data =============================
  ...
  a_laplace <- rep_len(1, simulationsize)
...
# ========================== Generating mix_var ================================    
    ...
    for (jj in nlevelsWT(wav_lat_var):1){
      ...
      parameters_a <- rgamma(1,
                             shape = (kappa+q),
                             rate = (xi+
                                       sum(abs(coef_wavelet[(2+r):(r+q+1)])))
      )
...
# ============================ Generating theta ================================  
    ...
    pre_tn <- rbinom(n = simulationsize, size = 1, prob = prob_tn)
    ...
    prob_l_tn <- (runif(n = simulationsize)*(upper_bound_tn - lower_bound_tn))+
      lower_bound_tn
    
    prob_l_tn[prob_l_tn == 1] <- (1 - epsilon)
    prob_l_tn[prob_l_tn == 0] <- epsilon
    
    coef_wavelet <- mix_var*qnorm(prob_l_tn, mean = mean_tn, sd = 1)
